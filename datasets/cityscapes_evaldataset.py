...
class CityscapesEvalDataset(MonoDataset):
    ...
    def get_color(self, city, frame_name, side, do_flip, is_sequence=False):
        ...
        if not os.path.exists(self.get_image_path(city, frame_name, side, is_sequence)):
            return False
    ...
    def get_doj_mask(self, city, frame_name, side, do_flip):
        ...
        frame = int(frame)
        mask = np.load(os.path.join(self.data_path, 'val_mask/{}_{}_{}.npy'.format(city, seq, frame)))
