...
class CityscapesPreprocessedDataset(MonoDataset):
    ...
    def load_intrinsics(self, city, frame_name):
        ...
        camera_file = os.path.join(self.data_path, 'train', city, "{}_cam.txt".format(frame_name))
    ...
    # added part!!
    def get_color_path(self, city, frame_name):
        image_path = os.path.join(city, frame_name)
        return image_path
    ...
    def get_doj_mask(self, city, frame_name, side, do_flip):
        ...
        mask = np.load(os.path.join(self.data_path, 'train_mask/{}_{}_{}.npy'.format(city, seq, frame)))
        maskm1 = np.load(os.path.join(self.data_path, 'train_mask/{}_{}_{}-1.npy'.format(city, seq, frame)))
        maskp1 = np.load(os.path.join(self.data_path, 'train_mask/{}_{}_{}+1.npy'.format(city, seq, frame)))
    ...
    def get_image_path(self, city, frame_name):
        return os.path.join(self.data_path, 'train', city, "{}.jpg".format(frame_name))
